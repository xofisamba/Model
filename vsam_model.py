...
# PySAM imports for enhanced modeling
try:
    from PySAM import Windpower, Pvwattsv8, Singleowner
    PYSAM_AVAILABLE = True
except ImportError:
    PYSAM_AVAILABLE = False
    warnings.warn("PySAM not available. Running in legacy mode.")

# ============================================================================
# PYSAM WRAPPER FUNCTIONS - vSAM ENHANCEMENTS
# ============================================================================
...
def get_pysam_solar_generation(capacity_dc_mw, location_data=None):
    """
    Calculate solar generation using PySAM Pvwattsv8 model.
    
    Args:
        capacity_dc_mw: DC capacity in MW
        location_data: optional dict with lat, lon, elevation
    """
    if not PYSAM_AVAILABLE:
        return None
    ...
    try:
        ...
        # Set capacity in kW
        pv.SystemDesign.system_capacity = capacity_dc_mw * 1000
        
        # Module type: 0=Standard, 1=Premium, 2=Thin film
        pv.SystemDesign.module_type = 0
        
        # Array type: 0=Fixed open rack, 1=Fixed roof mount, 2=1Axis, 3=Backtrack, 4=2Axis
        pv.SystemDesign.array_type = 0
        
        # Loss factors (default values)
        pv.SystemDesign.losses = 14  # Overall system losses %
        
        # Set location if provided
        if location_data:
            pv.SolarResource.lat = location_data.get('lat', 43.5)  # Default to Balkan
            pv.SolarResource.lon = location_data.get('lon', 18.0)
            pv.SolarResource.tz = 1  # CET
            pv.SolarResource.elev = location_data.get('elevation', 500)
        ...
        return {
            ...
            'annual_energy_mwh': pv.Outputs.ac_annual / 1000,
            'ac_monthly': pv.Outputs.ac_monthly.tolist() if hasattr(pv.Outputs, 'ac_monthly') else None,
            'kwh_per_kw': pv.Outputs.kwh_per_kw if hasattr(pv.Outputs, 'kwh_per_kw') else None
        }
    except Exception as e:
        warnings.warn(f"PySAM Pvwatts error: {e}")
        return None

def get_pysam_financials(energy_mwh, capacity_mw, tariff_eur_per_mwh, debt_percent=0.7, 
                         interest_rate=0.06, debt_tenor_years=12, tax_rate=0.1):
    """
    Run Singleowner financial model with PySAM.
    
    Returns: dict with IRR, NPV, LCOE, debt_service, etc.
    """
    if not PYSAM_AVAILABLE:
        return None
    
    try:
        # Create singleowner model
        single = Singleowner.new()
        
        # System output - connect to energy generation
        single.SystemOutput.annual_energy_input = energy_mwh * 1000  # kWh
        single.SystemOutput.system_capacity = capacity_mw * 1000  # kW
        
        # Financial parameters
        single.FinancialParameters.system_lifetime = 25
        single.FinancialParameters.analysis_period = 25
        single.FinancialParameters.debt_fraction = debt_percent
        single.FinancialParameters.loan_rate = interest_rate
        single.FinancialParameters.loan_term = debt_tenor_years
        
        # Electricity rates - PPA tariff
        single.ElectricityRates.rate = tariff_eur_per_mwh / 1000  # Convert to €/kWh
        single.ElectricityRates.rate_escalation = 0.02  # 2% annual escalation
        
        # Depreciation
        single.Depreciation.depreciation_type = 0  # MACRS
        
        # Tax
        single.FinancialParameters.federal_tax_rate = tax_rate
        single.FinancialParameters.state_tax_rate = 0
        
        # Execute
        single.execute()
        
        return {
            'irr': single.Outputs.analysis_period_irr if hasattr(single.Outputs, 'analysis_period_irr') else None,
            'npv': single.Outputs.npv if hasattr(single.Outputs, 'npv') else None,
            'lcoe': single.Outputs.lcoe_fcr if hasattr(single.Outputs, 'lcoe_fcr') else None,
            'levelized_cost': single.Outputs.levelized_cost_of_energy_main if hasattr(single.Outputs, 'levelized_cost_of_energy_main') else None,
            'annual_costs': single.Outputs.cf_annual_costs.tolist() if hasattr(single.Outputs, 'cf_annual_costs') else None,
            'annual_revenue': single.Outputs.cf_energy_sales.tolist() if hasattr(single.Outputs, 'cf_energy_sales') else None,
            'cf_debt_service': single.Outputs.cf_debt_service_total.tolist() if hasattr(single.Outputs, 'cf_debt_service_total') else None,
            'cf_ebitda': single.Outputs.cf_ebitda.tolist() if hasattr(single.Outputs, 'cf_ebitda') else None,
        }
    except Exception as e:
        warnings.warn(f"PySAM Singleowner error: {e}")
        return None
...
if 'repayment_frequency' not in st.session_state:
    st.session_state.repayment_frequency = 12  # months: 3, 6, or 12
if 'construction_start_date' not in st.session_state:
    st.session_state.construction_start_date = "2025-01-01"  # Default construction start
if 'construction_duration_months' not in st.session_state:
    st.session_state.construction_duration_months = 12  # 12 months construction
if 'investor_structures' not in st.session_state:
    st.session_state.investor_structures = [
        {'name': 'Sponsor', 'percentage': 100.0},
    ]
if 'arrangement_fee' not in st.session_state:
    st.session_state.arrangement_fee = 1.5  # % of debt
if 'commitment_fee' not in st.session_state:
    st.session_state.commitment_fee = 0.5  # % on undrawn
if 'margin' not in st.session_state:
    st.session_state.margin = 2.0  # % margin on base rate
...
# ============================================================================
    ...
    # Financial Parameters (Shared)
    st.markdown('<div class="section-header">FINANCIAL PARAMETERS (Shared)</div>', unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    with col1:
        st.session_state.repayment_frequency = st.selectbox(
            "Repayment Frequency",
            options=[3, 6, 12],
            index=2,
            format_func=lambda x: {3: "Quarterly (3 mo)", 6: "Semi-annual (6 mo)", 12: "Annual (12 mo)"}[x],
            key="repay_freq_shared"
        )
    with col2:
        st.write(f"**Current:** {st.session_state.repayment_frequency} months")
    with col3:
        st.write("")
    
    # Investor Structure
    st.markdown('<div class="section-header">👥 INVESTOR STRUCTURE</div>', unsafe_allow_html=True)
    investors = st.session_state.investor_structures
    
    for i, inv in enumerate(investors):
        col1, col2, col3 = st.columns([2, 1, 0.5])
        with col1:
            inv['name'] = st.text_input(f"Investor {i+1} Name", value=inv['name'], key=f"inv_name_{i}")
        with col2:
            inv['percentage'] = st.number_input(f"%", value=float(inv['percentage']), min_value=0.0, max_value=100.0, step=0.5, key=f"inv_pct_{i}")
        with col3:
            if st.button("🗑️", key=f"inv_del_{i}") and len(investors) > 1:
                investors.pop(i)
                st.rerun()
    
    if st.button("+ Add Investor", key="add_investor"):
        investors.append({'name': f'Investor {len(investors)+1}', 'percentage': 0.0})
        st.rerun()
    
    total_pct = sum(inv['percentage'] for inv in investors)
    if abs(total_pct - 100.0) > 0.1:
        st.warning(f"⚠️ Total percentage is {total_pct:.1f}% (should be 100%)")
    else:
        st.success(f"✅ Total: {total_pct:.1f}%")
    
    st.session_state.investor_structures = investors
    
    # Financing Parameters
    st.markdown('<div class="section-header">💰 FINANCING COSTS</div>', unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    with col1:
        st.session_state.arrangement_fee = st.number_input(
            "Arrangement Fee (%)",
            value=float(st.session_state.arrangement_fee),
            min_value=0.0, max_value=5.0, step=0.1,
            help="Fee charged by lenders for arranging the loan (% of debt)"
        )
    with col2:
        st.session_state.commitment_fee = st.number_input(
            "Commitment Fee (%)",
            value=float(st.session_state.commitment_fee),
            min_value=0.0, max_value=3.0, step=0.1,
            help="Fee on undrawn loan amount (% per year)"
        )
    with col3:
        st.session_state.margin = st.number_input(
            "Margin (%)",
            value=float(st.session_state.margin),
            min_value=0.0, max_value=10.0, step=0.1,
            help="Margin on top of base rate (% per year)"
        )
    
    total_interest_rate = st.session_state.base_rate * 100 + st.session_state.margin
    st.info(f"💡 Total Interest Rate: {st.session_state.base_rate*100:.2f}% (base) + {st.session_state.margin:.2f}% (margin) = {total_interest_rate:.2f}%")
    
    # Construction Timeline
    st.markdown('<div class="section-header">CONSTRUCTION TIMELINE</div>', unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    with col1:
        construction_start = st.date_input(
            "Start of Construction",
            value=datetime.strptime(st.session_state.construction_start_date, "%Y-%m-%d").date(),
            help="Date when construction begins"
        )
        st.session_state.construction_start_date = construction_start.strftime("%Y-%m-%d")
    with col2:
        duration_months = st.number_input(
            "Construction Duration (months)",
            value=int(st.session_state.construction_duration_months),
            min_value=1,
            max_value=36,
            step=1,
            help="Duration of construction period"
        )
        st.session_state.construction_duration_months = duration_months
    with col3:
        # Convert date to datetime for arithmetic
        start_datetime = datetime.combine(construction_start, datetime.min.time())
        cod_datetime = start_datetime + timedelta(days=duration_months * 30)
        cod_date = cod_datetime.date()
        st.session_state.cod_date = cod_date.strftime("%Y-%m-%d")
        st.success(f"**COD:** {cod_date.strftime('%d.%m.%Y')}")
        st.caption(f"📍 {construction_start.strftime('%d.%m.%Y')} → ⏱️ {duration_months} mo → 🚀")
...
# ============================================================================
    ...
    # Repayment frequency
    freq = st.session_state.get('repayment_frequency', 12)
    freq_labels = {3: "Quarterly", 6: "Semi-annual", 12: "Annual"}
    freq_label = freq_labels.get(freq, "Annual")
    
    # Get dates based on frequency
    def get_payment_dates(frequency):
        if frequency == 3:  # Quarterly
            return ['31.03', '30.06', '30.09', '31.12']
        elif frequency == 6:  # Semi-annual
            return ['30.06', '31.12']
        else:  # Annual
            return ['31.12']
    ...
    if rate > 0 and tenor > 0:
        ...
        for year in range(1, tenor + 1):
            if st.session_state.debt_sculpting and sculpted_schedule:
                annual_ds = sculpted_schedule[year - 1] if year <= len(sculpted_schedule) else 0
            else:
                annual_ds = calculate_annual_debt_service()
            
            # Split annual debt service into payment periods
            ds_per_payment = annual_ds / payments_per_year
            
            year_interest_total = 0
            year_principal_total = 0
            
            for i, date_str in enumerate(payment_dates):
                period_interest = balance * (rate / payments_per_year)  # Simple interest per period
                period_principal = ds_per_payment - period_interest
                balance -= period_principal
                year_interest_total += period_interest
                year_principal_total += period_principal
                
                detailed_data.append({
                    'Date': f"{date_str}.{year}",
                    'Year': year,
                    'Period': date_str,
                    'Opening Balance': balance + period_principal,
                    'Interest': period_interest,
                    'Principal': period_principal,
                    'Debt Service': ds_per_payment,
                    'Closing Balance': max(0, balance)
                })
            
            annual_summary.append({
                'Year': year,
                'Opening': debt if year == 1 else annual_summary[year-2]['Closing'],
                'Interest': year_interest_total,
                'Principal': year_principal_total,
        ...
        with col3:
            total_interest = sum(d['Interest'] for d in annual_summary)
            st.metric("Total Interest", f"{total_interest:,.0f} k€")
        with col4:
            total_ds = sum(d['Debt Service'] for d in annual_summary)
            st.metric("Total DS", f"{total_ds:,.0f} k€")
        
        # Tabs for detailed vs summary
        tab1, tab2 = st.tabs(["📅 Detailed Schedule", "📊 Annual Summary"])
        
        with tab1:
            st.markdown(f"#### Detailed Payment Schedule ({freq_label})")
            det_df = pd.DataFrame(detailed_data)
            display_det = det_df.copy()
            for col in ['Opening Balance', 'Interest', 'Principal', 'Debt Service', 'Closing Balance']:
                display_det[col] = display_det[col].apply(lambda x: f"{x:,.0f}")
            st.dataframe(display_det, use_container_width=True, hide_index=True, height=500)
        
        with tab2:
            st.markdown("#### Annual Summary")
            ann_df = pd.DataFrame(annual_summary)
            fig = make_subplots(specs=[[{"secondary_y": True}]])
            fig.add_trace(go.Bar(x=[f"Y{d['Year']}" for d in annual_summary], y=[d['Interest'] for d in annual_summary], name='Interest', marker_color='#C55A11'), secondary_y=False)
            fig.add_trace(go.Bar(x=[f"Y{d['Year']}" for d in annual_summary], y=[d['Principal'] for d in annual_summary], name='Principal', marker_color='#217346'), secondary_y=False)
            fig.add_trace(go.Scatter(x=[f"Y{d['Year']}" for d in annual_summary], y=[d['Closing'] for d in annual_summary], name='Balance', mode='lines+markers', line=dict(color='#00B0F0', width=3)), secondary_y=True)
            fig.update_layout(title='Annual Debt Service Summary', xaxis_title='Year', barmode='stack', template='plotly_white', height=400)
            ...
            for col in ['Opening', 'Interest', 'Principal', 'Debt Service', 'Closing']:
                display_ann[col] = display_ann[col].apply(lambda x: f"{x:,.0f}")
            st.dataframe(display_ann, use_container_width=True, hide_index=True, height=500)
