...
if 'gearing_ratio' not in st.session_state:
    st.session_state.gearing_ratio = 0.784
...
if 'depreciation_rate' not in st.session_state:
    st.session_state.depreciation_rate = 0.10
if 'depreciation_period' not in st.session_state:
    st.session_state.depreciation_period = 10  # Years over which asset is depreciated
...
if 'construction_period' not in st.session_state:
    st.session_state.construction_period = 2
...
def calculate_monte_carlo(sims=1000):
    ...
    for _ in range(sims):
        ...
        total_negative = abs(sum(c for c in cash_flows if c < 0))
        if total_negative > 0:
            irr = (total_positive / total_negative - 1) * 100 / (st.session_state.investment_horizon / 2)
            results.append(irr)
...
sheet_tabs = [
    ...
    "💵 Cash Flow", "🏦 Debt Service", "💎 Equity", "🚩 Flags", "📤 Outputs", "⚙️ Advanced"
]
...
# ============================================================================
    ...
    st.markdown('<div class="section-header">TECHNICAL PARAMETERS</div>', unsafe_allow_html=True)
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        st.session_state.ppa_base_tariff = st.number_input("PPA Base Tariff (€/MWh)", value=float(st.session_state.ppa_base_tariff), min_value=1.0, max_value=300.0, step=1.0)
    with col2:
    ...
    with col3:
        st.session_state.ppa_term = st.number_input("PPA Term (years)", value=int(st.session_state.ppa_term), min_value=5, max_value=30, step=1)
    
    # Merchant Tail (Wind only)
    if tech == "Wind":
        st.markdown("**Merchant Tail (Post-PPA)**")
        col1, col2, col3 = st.columns(3)
        with col1:
            st.session_state.merchant_tail_enabled = st.checkbox("Enable Merchant Tail", value=st.session_state.merchant_tail_enabled)
        if st.session_state.merchant_tail_enabled:
            with col2:
                st.session_state.merchant_price = st.number_input("Merchant Price (€/MWh)", value=float(st.session_state.merchant_price), min_value=10.0, max_value=200.0, step=5.0)
            with col3:
                st.metric("Post-PPA Period", f"Year {st.session_state.ppa_term + 1} - {st.session_state.investment_horizon}")
    
    # Financing Parameters
    st.markdown('<div class="section-header">FINANCING PARAMETERS</div>', unsafe_allow_html=True)
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.session_state.gearing_ratio = st.number_input("Gearing Ratio (%)", value=float(st.session_state.gearing_ratio * 100), min_value=0.0, max_value=100.0, step=1.0) / 100
    with col2:
        st.session_state.debt_tenor = st.number_input("Debt Tenor (years)", value=int(st.session_state.debt_tenor), min_value=5, max_value=25, step=1)
    with col3:
        st.session_state.target_dscr = st.number_input("Target DSCR", value=float(st.session_state.target_dscr), min_value=1.0, max_value=2.5, step=0.05)
    with col4:
        st.session_state.depreciation_period = st.number_input("Depr. Period (years)", value=int(st.session_state.depreciation_period), min_value=1, max_value=st.session_state.investment_horizon, step=1)
    
    if tech == "Wind":
        col1, col2, col3 = st.columns(3)
        with col1:
            st.session_state.senior_debt_margin = st.number_input("Senior Debt Margin (%)", value=float(st.session_state.senior_debt_margin * 100), min_value=0.0, max_value=500.0, step=10.0) / 100
        with col2:
            st.session_state.shl_rate = st.number_input("SHL Rate (%)", value=float(st.session_state.shl_rate * 100), min_value=0.0, max_value=20.0, step=0.5) / 100
        with col3:
            st.session_state.p99_debt_sizing = st.checkbox("Use P99 for Debt Sizing", value=st.session_state.p99_debt_sizing)
    
    # Battery Storage (BESS)
    st.markdown('<div class="section-header">🔋 BATTERY STORAGE (BESS)</div>', unsafe_allow_html=True)
    
    col1, col2, col3 = st.columns(3)
    with col1:
        st.session_state.bess_enabled = st.checkbox("Enable Battery Storage", value=st.session_state.bess_enabled)
    
    if st.session_state.bess_enabled:
        col1, col2, col3 = st.columns(3)
        with col1:
            st.session_state.bess_capacity_mwh = st.number_input("Battery Capacity (MWh)", value=float(st.session_state.bess_capacity_mwh), min_value=0.1, max_value=1000.0, step=0.5)
        with col2:
            st.session_state.bess_power_mw = st.number_input("Battery Power (MW)", value=float(st.session_state.bess_power_mw), min_value=0.1, max_value=500.0, step=0.5)
        with col3:
            st.session_state.bess_cost_per_mwh = st.number_input("Battery Cost (€/MWh)", value=float(st.session_state.bess_cost_per_mwh), min_value=50000.0, max_value=500000.0, step=10000.0)
        
        col1, col2, col3 = st.columns(3)
        with col1:
            st.session_state.bess_roundtrip_efficiency = st.number_input("Round-trip Efficiency (%)", value=float(st.session_state.bess_roundtrip_efficiency * 100), min_value=70.0, max_value=99.0, step=1.0) / 100
        with col2:
            st.session_state.bess_cycle_life = st.number_input("Cycle Life (cycles)", value=int(st.session_state.bess_cycle_life), min_value=1000, max_value=15000, step=500)
        with col3:
            st.session_state.bess_annual_cycles = st.number_input("Annual Cycles", value=int(st.session_state.bess_annual_cycles), min_value=1, max_value=1000, step=10)
        
        col1, col2, col3 = st.columns(3)
        with col1:
            st.session_state.bess_min_soc = st.number_input("Min SOC (%)", value=float(st.session_state.bess_min_soc * 100), min_value=5.0, max_value=50.0, step=1.0) / 100
        with col2:
            st.session_state.bess_max_soc = st.number_input("Max SOC (%)", value=float(st.session_state.bess_max_soc * 100), min_value=50.0, max_value=99.0, step=1.0) / 100
        with col3:
            st.session_state.bess_degradation_rate = st.number_input("Annual Degradation (%)", value=float(st.session_state.bess_degradation_rate * 100), min_value=0.1, max_value=10.0, step=0.1) / 100
        
        # BESS summary
        bess_capex = st.session_state.bess_capacity_mwh * st.session_state.bess_cost_per_mwh / 1000  # kEUR
        st.info(f"🔋 Battery CAPEX: **{bess_capex:,.0f} kEUR** | Energy: **{bess_capex/st.session_state.bess_power_mw:.1f}** kEUR/MW")
...
# ============================================================================
    ...
    for year in years:
        ...
        depreciation = calculate_total_capex() * st.session_state.depreciation_rate
        taxable = max(0, ebitda - depreciation)
        tax = taxable * st.session_state.corporate_tax_rate if year > st.session_state.ppa_term or tech == "Solar" else 0
        net_inc = taxable - tax
        ...
        pl_data.append({
            ...
            'Tax': -tax,
            'Net Income': net_inc,
            'DSCR': ebitda / get_debt_service(year) if get_debt_service(year) > 0 else float('nan')
        })
    ...
    fig.add_trace(go.Bar(
        ...
        name='Revenue',
        marker_color='#1B5E3B',
        hovertemplate='<b>Revenue</b><br>Year %{x}<br>%{y:,.0f} k€<extra></extra>'
    ))
    ...
    fig.add_trace(go.Bar(
        ...
        name='OPEX',
        marker_color='#C55A11',
        hovertemplate='<b>OPEX</b><br>Year %{x}<br>%{y:,.0f} k€<extra></extra>'
    ))
    ...
    fig = apply_chart_style(fig, title='P&L - Revenue, OPEX & EBITDA', yaxis_title='k€', barmode='group', height=400)
    st.plotly_chart(fig, use_container_width=True)
    
    # Table
    st.markdown("---")
    display_pl = pl_df.copy()
    for col in ['Revenue', 'OPEX', 'EBITDA', 'Depreciation', 'Taxable Income', 'Tax', 'Net Income']:
...
# ============================================================================
    ...
    depreciation_period = st.session_state.depreciation_period
    total_debt = calculate_debt_amount()
    equity_initial = calculate_equity_amount()
    ...
    for year in years:
        if year == 0:
            gross_fa = gfa
            accum_depr = 0
            net_fa = gross_fa - accum_depr
            cash = 0
            equity = equity_initial
            senior_debt = total_debt
        else:
            # Depreciate only if within depreciation period and there's remaining value
            ...
            taxable = max(0, calculate_ebitda(year) - annual_depr)
            net_inc = taxable * (1 - st.session_state.corporate_tax_rate)
            reserves += net_inc
            
            equity = equity_initial + reserves
            senior_debt = total_debt * max(0, (st.session_state.debt_tenor - year + 1)) / st.session_state.debt_tenor
            cash = 0
        
        total_assets = net_fa + cash
        total_liabilities = equity + senior_debt
    ...
    # Chart
    fig = go.Figure()
    fig.add_trace(go.Bar(x=[d['Year'] for d in bs_data], y=[d['Net FA'] for d in bs_data], name='Net Fixed Assets', marker_color='#217346'))
    fig.add_trace(go.Bar(x=[d['Year'] for d in bs_data], y=[d['Senior Debt'] for d in bs_data], name='Senior Debt', marker_color='#C55A11'))
    fig.add_trace(go.Scatter(x=[d['Year'] for d in bs_data], y=[d['Equity'] for d in bs_data], name='Equity', mode='lines+markers', line=dict(color='#00B0F0', width=3)))
    fig.update_layout(title='Balance Sheet Structure', xaxis_title='Period', yaxis_title='k€', barmode='group', template='plotly_white', height=400)
    st.plotly_chart(fig, use_container_width=True)
    
    # Table
    st.markdown("---")
    display_bs = bs_df.copy()
    for col in ['Gross FA', 'Accum Depr', 'Net FA', 'Cash', 'Total Assets', 'Equity', 'Senior Debt', 'Total L&E']:
...
# ============================================================================
...
elif st.session_state.active_sheet == "🚩 Flags":
    st.header("🚩 Flags - Configuration Options")
    ...
    if 'flags' not in st.session_state:
        st.session_state.flags = {
            ...
            'ppa_life': 10,
            'senior_debt_maturity': 12,
            'depreciation_period': 30,
            'construction_period': 12,
            'annual_period': True,
            'use_p90_yield': True,
            'apply_inflation': True,
            'apply_tariff_escalation': True,
            'thin_capitalization_check': True,
            'max_debt_ratio': 80.0,
        }
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        st.number_input("Project Life (years)", value=30, min_value=10, max_value=50, key="flag_project_life")
    with col2:
        st.number_input("Construction Period (months)", value=12, min_value=6, max_value=36, key="flag_construction")
    with col3:
        st.checkbox("Apply Inflation", value=True, key="flag_inflation")
    
    st.markdown("**Model Options**")
    col1, col2, col3 = st.columns(3)
    with col1:
        st.checkbox("Use P90 Yield", value=True, key="flag_p90")
    with col2:
        st.checkbox("Apply Tariff Escalation", value=True, key="flag_escalation")
    with col3:
        st.number_input("Max Debt Ratio (%)", value=80.0, min_value=50.0, max_value=95.0, key="flag_max_debt")
...
# ============================================================================
    ...
    # ===== MONTE CARLO =====
        ...
        if 'mc_results' in st.session_state:
            ...
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Mean IRR", f"{mc['mean']:.1f}%")
            with col2:
                st.metric("Median IRR", f"{mc['p50']:.1f}%")
            with col3:
                st.metric("P10 IRR", f"{mc['p10']:.1f}%")
            with col4:
                st.metric("P90 IRR", f"{mc['p90']:.1}%")
            
            st.markdown("---")
            
            fig = go.Figure()
            fig.add_trace(go.Histogram(x=mc['all_results'], nbins=50, name='IRR Distribution', marker_color='#217346'))
            fig.add_vline(x=mc['mean'], line_dash="dash", line_color="red", annotation_text=f"Mean: {mc['mean']:.1f}%")
    ...
    # ===== CASH SWEEP =====
        ...
        col1, col2 = st.columns(2)
        with col1:
            enabled = st.checkbox("Enable Cash Sweep", value=st.session_state.cash_sweep_enabled)
        with col2:
            threshold = st.slider("DSCR Threshold", min_value=1.0, max_value=2.0, value=1.20, step=0.05)
        
        years_list = list(range(1, min(16, st.session_state.debt_tenor + 1)))
        sweep_data = []
        ...
        for year in years_list:
            ...
            if enabled and dscr > threshold:
                excess = ebitda - (debt_service * threshold)
            else:
                excess = 0
            
            sweep_data.append({'Year': year, 'EBITDA': f"{ebitda:,.0f}", 'DSCR': f"{dscr:.2f}x", 'Excess Cash': f"{excess:,.0f}"})
...
def load_version(version_data):
    ...
    st.session_state.gearing_ratio = version_data.get('gearing_ratio', 0.784)
