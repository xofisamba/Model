...
# ============================================================================
    ...
    # Auto-calculate depreciation rate based on investment horizon
    ...
    shareholder_loan_initial = calculate_equity_amount()
    ...
    shareholder_loan_balance = shareholder_loan_initial
    cash_balance = 0
    ...
    for year in years:
        if year == 0:
            ...
            net_fa = gross_fa
            cash = 0
            sh_loan = shareholder_loan_initial
            ret_earn = 0
            total_equity = ret_earn
            sr_debt = total_debt
            ...
            # Calculate cash and debt service
            ...
            # Tax: depreciation reduces taxable income, then apply tax rate
            taxable_income = max(0, ebitda - annual_depr)
            tax = taxable_income * tax_rate
            
            # Net income = EBITDA - Tax (depreciation already accounted in tax)
            net_inc = ebitda - tax
            retained_earnings += net_inc
            
            # FCF after debt service for cash accumulation and debt repayment
            fcf = ebitda - annual_ds  # Cash after senior debt service
            
            # Repayment order: 1) Senior Debt per schedule, 2) Shareholder Loan if remains
            if fcf > 0 and senior_debt_balance > 0:
                sr_repay = min(fcf, senior_debt_balance)
                senior_debt_balance -= sr_repay
                fcf -= sr_repay
            
            if fcf > 0 and shareholder_loan_balance > 0:
                sh_repay = min(fcf, shareholder_loan_balance)
                shareholder_loan_balance -= sh_repay
                fcf -= sh_repay
            
            # Remaining FCF goes to cash
    ...
    fig.add_trace(go.Bar(x=[d['Year'] for d in bs_data], y=[d['Net FA'] for d in bs_data], name='Net Fixed Assets', marker_color='#217346', offset=0), secondary_y=False)
    fig.add_trace(go.Bar(x=[d['Year'] for d in bs_data], y=[d['Cash'] for d in bs_data], name='Cash', marker_color='#4CAF50', offset=0), secondary_y=False)
    fig.add_trace(go.Bar(x=[d['Year'] for d in bs_data], y=[d['Retained Earnings'] for d in bs_data], name='Retained Earnings (Equity)', marker_color='#00B0F0', offset=0), secondary_y=False)
    fig.add_trace(go.Bar(x=[d['Year'] for d in bs_data], y=[d['Shareholder Loan'] for d in bs_data], name='Shareholder Loan (Liab)', marker_color='#FFC107', offset=0), secondary_y=False)
    fig.add_trace(go.Bar(x=[d['Year'] for d in bs_data], y=[d['Senior Debt'] for d in bs_data], name='Senior Debt (Liab)', marker_color='#C55A11', offset=0), secondary_y=False)
    fig.update_layout(title='Balance Sheet (Assets = Liabilities)', xaxis_title='Period', yaxis_title='k€', barmode='stack', template='plotly_white', height=500)
    st.plotly_chart(fig, use_container_width=True)
    
    # Total Assets vs Liabilities chart
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=[d['Year'] for d in bs_data], y=[d['Total Assets'] for d in bs_data], name='Total Assets', mode='lines+markers', line=dict(color='#217346', width=3)))
    fig2.add_trace(go.Scatter(x=[d['Year'] for d in bs_data], y=[d['Total Liabilities'] for d in bs_data], name='Total Liabilities', mode='lines+markers', line=dict(color='#C55A11', width=3)))
    fig2.update_layout(title='Total Assets = Total Liabilities', xaxis_title='Period', yaxis_title='k€', template='plotly_white', height=350)
    st.plotly_chart(fig2, use_container_width=True)
    
    # Table
    st.markdown("---")
    st.markdown(f"#### 📋 Balance Sheet (k€) - Depr: {depreciation_rate*100:.2f}%/yr | Tax: {tax_rate*100:.0f}%")
    
    display_bs = bs_df.copy()
    for col in ['Gross FA', 'Accum Depr', 'Net FA', 'Cash', 'Total Assets', 'Retained Earnings', 'Total Equity', 'Shareholder Loan', 'Senior Debt', 'Total Liabilities']:
        display_bs[col] = display_bs[col].apply(lambda x: f"{x:,.0f}")
    display_bs['Check'] = display_bs['Check'].apply(lambda x: f"{x:,.0f}")
    
    rename_cols = {
        'Gross FA': 'Gross FA',
        'Accum Depr': 'Accum Depr',
        'Net FA': 'Net FA',
        'Cash': 'Cash',
        'Total Assets': 'Total Assets',
        'Retained Earnings': 'Ret. Earn',
        'Total Equity': 'Total Equity',
        'Shareholder Loan': 'Sh. Loan',
        'Senior Debt': 'Sr. Debt',
        'Total Liabilities': 'Total Liab',
        'Check': 'Check'
    }
    display_bs = display_bs.rename(columns=rename_cols)
